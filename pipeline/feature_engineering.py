...
def perform_feature_engineering(df: pd.DataFrame, target: str, problem_type: str) -> tuple[pd.DataFrame, pd.Series, str]:
    """
    Separates target, creates additional features (interactions/polynomials) if applicable.
    Returns X (features), y (target), and a report.
    """
    report = []
    ...
    if categorical_cols:
        ...
        report.append(f"Applied One-Hot Encoding to: {', '.join(categorical_cols)}. Total features now: {X.shape[1]}")
